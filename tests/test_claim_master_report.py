...
def test_splitter_reads_renumbering_and_cancellation():
    text = (
        "11. The method of claim 1, wherein a is b.\n"
        "[12.] The cancelled method of claim 1.\n"
        "12.[13.] The method of claim 1, wherein c is d.\n"
    )
    claims = ClaimSplitter().split(text)

    assert [(c.number, c.old_number, c.deleted) for c in claims] == [
        (11, None, False),
        (12, None, True),
        (12, 13, False),
    ]
...
def test_clean_sections_say_no_errors_found():
    ...
    assert section_status(report, ReportSection.ANTECEDENTS).kind == "warning"
